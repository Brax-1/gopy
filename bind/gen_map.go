...
`)
		...
		g.pywrap.Indent()
		g.pywrap.Printf("_%s_set(self.gopyvar, k, v)\n", qNm)
		g.pywrap.Outdent()
		g.pywrap.Outdent()
		g.pywrap.Outdent()
		g.pywrap.Outdent()
		...
		if ksym.hasGopyvar() {
			if esym.hasGopyvar() {
				g.pywrap.Printf("return %s(gopyvar=_%s_elem(self.gopyvar, key.gopyvar))\n", esym.pyPkgId(slc.gopkg), qNm)
			} else {
				g.pywrap.Printf("return _%s_elem(self.gopyvar, key.gopyvar)\n", qNm)
			}
		} else {
			if esym.hasGopyvar() {
		...
		if esym.hasGopyvar() {
			if ksym.hasGopyvar() {
				g.pywrap.Printf("_%s_set(self.gopyvar, key.gopyvar, value.gopyvar)\n", qNm)
			} else {
				g.pywrap.Printf("_%s_set(self.gopyvar, key, value.gopyvar)\n", qNm)
			}
		} else {
			if ksym.hasGopyvar() {
				g.pywrap.Printf("_%s_set(self.gopyvar, key.gopyvar, value)\n", qNm)
			} else {
				g.pywrap.Printf("_%s_set(self.gopyvar, key, value)\n", qNm)
			}
		}
		g.pywrap.Outdent()
		...
		g.pywrap.Indent()
		if ksym.hasGopyvar() {
			g.pywrap.Printf("return _%s_delete(self.gopyvar, key.gopyvar)\n", qNm)
		} else {
			g.pywrap.Printf("return _%s_delete(self.gopyvar, key)\n", qNm)
		}
		g.pywrap.Outdent()

		g.pywrap.Printf("def keys(self):\n")
		g.pywrap.Indent()
		g.pywrap.Printf("return %s(gopyvar=_%s_keys(self.gopyvar))\n", keyslnm, qNm)
		g.pywrap.Outdent()
		...
		g.pywrap.Indent()
		if ksym.hasGopyvar() {
			g.pywrap.Printf("return _%s_contains(self.gopyvar, key.gopyvar)\n", qNm)
		} else {
			g.pywrap.Printf("return _%s_contains(self.gopyvar, key)\n", qNm)
		}
		g.pywrap.Outdent()
		...
		// g.pywrap.Printf("return _%s_elem(self.gopyvar, self.index)\n", qNm)
		// g.pywrap.Outdent()
	}
	...
	if !extTypes || !pyWrapOnly {
		...
		g.gofile.Printf("func %s_elem(gopyvar CGoGopyvar, _ky %s) %s {\n", slNm, ksym.cgoname, esym.cgoname)
		g.gofile.Indent()
		...
		g.gofile.Printf("}\n")
		if esym.go2py != "" {
			// If the go2py starts with gopyvarFromPtr_, use &v, otherwise just v
			val_str := ""
		...
		g.pybuild.Printf("mod.add_function('%s_elem', retval('%s'), [param('%s', 'gopyvar'), param('%s', '_ky')])\n", slNm, esym.cpyname, PyGopyvar, ksym.cpyname)

		// contains
		g.gofile.Printf("//export %s_contains\n", slNm)
		g.gofile.Printf("func %s_contains(gopyvar CGoGopyvar, _ky %s) C.char {\n", slNm, ksym.cgoname)
		g.gofile.Indent()
		g.gofile.Printf("s := deptrFromGopyvar_%s(gopyvar)\n", slNm)
		if ksym.py2go != "" {
		...
		g.pybuild.Printf("mod.add_function('%s_contains', retval('bool'), [param('%s', 'gopyvar'), param('%s', '_ky')])\n", slNm, PyGopyvar, ksym.cpyname)

		// set
		g.gofile.Printf("//export %s_set\n", slNm)
		g.gofile.Printf("func %s_set(gopyvar CGoGopyvar, _ky %s, _vl %s) {\n", slNm, ksym.cgoname, esym.cgoname)
		g.gofile.Indent()
		g.gofile.Printf("s := deptrFromGopyvar_%s(gopyvar)\n", slNm)
		if ksym.py2go != "" {
		...
		g.pybuild.Printf("mod.add_function('%s_set', None, [param('%s', 'gopyvar'), param('%s', 'key'), param('%s', 'value')])\n", slNm, PyGopyvar, ksym.cpyname, esym.cpyname)

		// delete
		g.gofile.Printf("//export %s_delete\n", slNm)
		g.gofile.Printf("func %s_delete(gopyvar CGoGopyvar, _ky %s) {\n", slNm, ksym.cgoname)
		g.gofile.Indent()
		g.gofile.Printf("s := deptrFromGopyvar_%s(gopyvar)\n", slNm)
		if ksym.py2go != "" {
		...
		g.pybuild.Printf("mod.add_function('%s_delete', None, [param('%s', 'gopyvar'), param('%s', '_ky')])\n", slNm, PyGopyvar, ksym.cpyname)

		// keys
		g.gofile.Printf("//export %s_keys\n", slNm)
		g.gofile.Printf("func %s_keys(gopyvar CGoGopyvar) CGoGopyvar {\n", slNm)
		g.gofile.Indent()
		g.gofile.Printf("s := deptrFromGopyvar_%s(gopyvar)\n", slNm)
		...
		g.pybuild.Printf("mod.add_function('%s_keys', retval('%s'), [param('%s', 'gopyvar')])\n", slNm, keyslsym.cpyname, PyGopyvar)
