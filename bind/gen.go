...
const (
	// GoGopyvar is the type to use for the Gopyvar map key, go-side
	GoGopyvar = "int64"
	// CGoGopyvar is Gopyvar for cgo files
	CGoGopyvar = "C.longlong"
	// PyGopyvar is within python
	PyGopyvar = "int64_t"
)
...
// 3 = libcfg, 4 = GoGopyvar, 5 = CGoGopyvar, 6 = all imports, 7 = mainstr, 8 = exe pre C, 9 = exe pre go
const (
	goPreamble = `/*
...
static inline void gopy_err_gopyvar() {
...
// type for the gopyvar -- int64 for speed (can switch to string)
type GoGopyvar %[4]s
type CGoGopyvar %[5]s
...
func DecRef(gopyvar CGoGopyvar) {
	gopyh.DecRef(gopyh.CGoGopyvar(gopyvar))
}

// IncRef increments the reference count for the specified gopyvar.
//export IncRef
func IncRef(gopyvar CGoGopyvar) {
	gopyh.IncRef(gopyh.CGoGopyvar(gopyvar))
}

// NumGopyvars returns the number of gopyvars currently in use.
//export NumGopyvars
func NumGopyvars() int {
	return gopyh.NumGopyvars()
}
...
mod.add_function('DecRef', None, [param('int64_t', 'gopyvar')])
mod.add_function('IncRef', None, [param('int64_t', 'gopyvar')])
mod.add_function('NumGopyvars', retval('int'), [])
`
...
`, pycfg.CFlags+exflags, ldflags)
	...
	g.gofile.Printf(goPreamble, g.cfg.Name, g.cfg.Cmd, libcfg, GoGopyvar, CGoGopyvar,
