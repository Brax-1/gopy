...
func (g *pyGen) genFuncSig(sym *symbol, fsym *Func) bool {
	...
	ifcgopyvar, gdoc := isIfaceGopyvar(gdoc)
	...
	if isMethod {
		goArgs = append(goArgs, "_gopyvar CGoGopyvar")
		pyArgs = append(pyArgs, fmt.Sprintf("param('%s', '_gopyvar')", PyGopyvar))
	...
	for i := 0; i < nargs; i++ {
		...
		if ifcgopyvar && arg.sym.goname == "interface{}" {
			goArgs = append(goArgs, fmt.Sprintf("%s %s", anm, CGoGopyvar))
			pyArgs = append(pyArgs, fmt.Sprintf("param('%s', '%s')", PyGopyvar, anm))
		} else {
...
func isIfaceGopyvar(gdoc string) (bool, string) {
	const PythonIface = "gopy:interface=gopyvar"
...
func (g *pyGen) genFuncBody(sym *symbol, fsym *Func) {
	...
	if isMethod {
		g.gofile.Printf(
			`vifc, __err := gopyh.VarFromGopyvarrrrTry((gopyh.CGoGopyvarrrrr)(_gopyvar), "%s")
if __err != nil {
`, symNm)
		g.gofile.Indent()
		if nres > 0 {
	...
	wrapArgs := []string{}
	if isMethod {
		wrapArgs = append(wrapArgs, "self.gopyvar")
	}
	for i, arg := range args {
		na := ""
		anm := pySafeArg(arg.Name(), i)
		switch {
		case ifcgopyvar && arg.sym.goname == "interface{}":
			na = fmt.Sprintf(`gopyh.VarFromGopyvarr((gopyh.CGoGopyvarrrr)(%s), "interface{}")`, anm)
		...
		case arg.sym.goname == "interface{}":
			if ifcgopyvar {
				wrapArgs = append(wrapArgs, fmt.Sprintf("%s.gopyvar", anm))
			} else {
				wrapArgs = append(wrapArgs, anm)
			}
		case arg.sym.hasGopyvar():
			wrapArgs = append(wrapArgs, fmt.Sprintf("%s.gopyvar", anm))
		default:
			wrapArgs = append(wrapArgs, anm)
		}
	...
	if nres > 0 {
		ret := res[0]
		if !rvIsErr && ret.sym.hasGopyvar() {
			rvHasGopyvar = true
			cvnm := ret.sym.pyPkgId(g.pkg.pkg)
			g.pywrap.Printf("return %s(gopyvar=_%s.%s(", cvnm, pkgname, mnm)
		} else {
	...
	if nres > 0 {
		...
		case ret.sym.hasGopyvar() && !ret.sym.isPtrOrIface():
			hasAddrOfTmp = true
	...
	if rvIsErr || nres == 2 {
		...
		if rvIsErr {
			...
			ret := res[0]
			if ret.sym.go2py != "" {
				if ret.sym.hasGopyvar() && !ret.sym.isPtrOrIface() {
